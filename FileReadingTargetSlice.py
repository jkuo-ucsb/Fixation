...
def getCurrentSlice(s_time, e_time, condition):
    currentslice = 1;
    time = 0
    for i in range(0, len(mes_time_list[condition]["dirtime"])):   
        ...
        if time > e_time:
            break
        if s_time <= time and time <= e_time:
...
for session in range(1,session_max+1): #Loops every session
    ...
    while i < len(mes_time_list[session]["fixtime"]):
        ...
        for sac in range(sac_count,len(sac_end)):
            if (sac_end[sac]["session_id"] == 2):
                sactime = sac_end[sac]["time"]
                print(str(s_time) + " " + str(sactime) + " " + str(e_time))
                if sactime > e_time:
                    break
                if s_time < sactime and sactime < e_time:
                    if (trial == 4):
                        print('mafia')
                    if (getCurrentSlice(s_time,e_time,session)) == targetslice:
                        if (trial == 4):
                            print('crazy')
